import type { ApplicationCreateDto, ApplicationDocumentedUpdateDto, ApplicationDto, ApplicationExcelDownloadDto, ApplicationReservationUpdateDto, ApplicationTenantDto, ApplicationUpdateDto, ApplicationWithNavigationPropertiesDto, GetApplicationsInput } from './models';
...
export class ApplicationService {
  ...
  deleteAll = (input: GetApplicationsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, void>({
      ...
      params: { filterText: input.filterText, notes: input.notes, refCode: input.refCode, applicationStatus: input.applicationStatus, reservationDateMin: input.reservationDateMin, reservationDateMax: input.reservationDateMax, reservationTimeMin: input.reservationTimeMin, reservationTimeMax: input.reservationTimeMax, trackingCode: input.trackingCode, isDocumented: input.isDocumented, customerId: input.customerId, sourceId: input.sourceId, outSourceId: input.outSourceId, countryId: input.countryId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
  ...
  getListAsExcelFile = (input: ApplicationExcelDownloadDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, Blob>({
      ...
      params: { downloadToken: input.downloadToken, filterText: input.filterText, notes: input.notes, refCode: input.refCode, applicationStatus: input.applicationStatus, reservationDateMin: input.reservationDateMin, reservationDateMax: input.reservationDateMax, reservationTimeMin: input.reservationTimeMin, reservationTimeMax: input.reservationTimeMax, trackingCode: input.trackingCode, isDocumented: input.isDocumented, customerId: input.customerId, sourceId: input.sourceId, outSourceId: input.outSourceId, countryId: input.countryId },
    },
    { apiName: this.apiName,...config });
